...
function unlink(unlinkConfig) {
    ...
    config.forEach(c => {
        const pj = JSON.parse(shelljs_1.cat(util_1.getPackageJsonPath(unlinkConfig, c.path)));
        Object.keys(pj.dependencies || {}).filter(d => config.find(c => c.name === d)).forEach(d => {
            if (unlinkConfig.version === UnlinkVersion.local) {
                ...
                const targetPackageJson = JSON.parse(shelljs_1.cat(util_1.getPackageJsonPath(unlinkConfig, targetConfig.path)));
